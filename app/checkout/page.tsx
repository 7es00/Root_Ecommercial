...
const CheckoutPage = () => {
  ...
  const makePurchase = async () => {
    ...
    try {
      // 1) أضف المنتجات إلى سلة RouteMisr أولاً
      for (const product of products) {
        // Add product multiple times based on quantity
        for (let i = 0; i < product.amount; i++) {
          const addToCartRes = await apiClient.cart.add(product.id);
          if (!addToCartRes.ok) {
      ...
      // 2) اجلب سلة المستخدم للحصول على cartId
      const cartRes = await apiClient.cart.get();
      const cartJson = await cartRes.json();
      const cartId: string | undefined = cartJson?.data?._id || cartJson?._id;
      
      if (!cartRes.ok || !cartId) {
        toast.error("Cannot load cart. Please add items first.");
        throw new Error("No cartId");
      }

      // 3) بيانات عنوان الشحن
      const shippingAddress = {
        details: checkoutForm.adress.trim(),
      ...
      if (paymentMethod === 'stripe') {
        ...
        const sessionRes = await apiClient.orders.createCheckoutSession(cartId, url, shippingAddress);
        const sessionJson = await sessionRes.json();

        if (!sessionRes.ok) {
          toast.error(sessionJson?.message || 'Failed to start checkout session');
          throw new Error('checkout-session failed');
        }

        const redirectUrl = sessionJson?.session?.url || sessionJson?.url;
        if (redirectUrl) {
        ...
        // دفع كاش عند التسليم
        console.log('Creating cash order with:', { cartId, shippingAddress });
        const cashRes = await apiClient.orders.createCash(cartId, shippingAddress);
        console.log('Cash order response:', cashRes);
        
        if (cashRes.ok) {
          const cashData = await cashRes.json();
          console.log('Cash order data:', cashData);
          toast.success('Cash on delivery order created successfully! You will be contacted for payment.');
          clearCart();
          router.push('/allorders');
          return;
        } else {
          const errorData = await cashRes.json();
          console.error('Cash order error:', errorData);
          toast.error(errorData?.message || 'Cash order failed');
          throw new Error('Cash order failed');
        }
      }
    } catch (error: any) {
                  ...
                  <label htmlFor="stripe-payment" className="ml-3 block text-sm font-medium text-gray-700">
                    <div className="flex items-center">
                      ...
                      <div className="ml-2 flex space-x-1">
                        <Image src="/visa.svg" width={20} height={20} alt="Visa" />
                        <Image src="/mastercard.svg" width={20} height={20} alt="Mastercard" />
                        <Image src="/ae.svg" width={20} height={20} alt="American Express" />
                      </div>
                    </div>
                ...
                <div className="flex items-center">
                  ...
                  <label htmlFor="cash-payment" className="ml-3 block text-sm font-medium text-gray-700">
                    <div className="flex items-center">
                      ...
                      <span className="ml-2 text-xs bg-green-100 text-green-800 px-2 py-1 rounded">COD</span>
                    </div>
                  ...
                  <div className="ml-3">
                    <h3 className="text-sm font-medium text-blue-800">
                      ...
                      <div className="mt-2 text-sm text-blue-700">
                        <p>You will be redirected to Stripe&apos;s secure payment page. Use test card numbers for development:</p>
                        <div className="mt-2 text-xs">
                          <p><strong>Visa:</strong> 4242 4242 4242 4242</p>
                          <p><strong>Mastercard:</strong> 5555 5555 5555 4444</p>
                          <p><strong>Amex:</strong> 3782 822463 10005</p>
                          <p><strong>CVC:</strong> Any 3 digits | <strong>Expiry:</strong> Any future date</p>
                        </div>
                      </div>
                    </div>
                  </div>
...
// Checkout wiring example (kept for docs; not used)
async function createCashOrder(cartId: string, shippingAddress:any){
  const res = await apiClient.orders.createCash(cartId, shippingAddress);
  return res.json?.() ?? res;
}
