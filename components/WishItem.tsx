...
const WishItem = ({
  ...
  const deleteItemFromWishlist = async (productId: string) => {
    try {
      const response = await apiClient.wishlist.remove(productId);
      ...
      if (response.ok) {
        ...
        toast.error(errorData.message || "Failed to remove item from wishlist");
      }
    } catch (error) {
  ...
  return (
    <tr className="hover:bg-gray-100 cursor-pointer">
      ...
      <td>
        <button className="btn btn-xs bg-blue-500 text-white border border-blue-500 hover:bg-white hover:text-blue-500 text-sm">
          <FaHeartCrack />
          <span
